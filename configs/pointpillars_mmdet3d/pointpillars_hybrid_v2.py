# PointPillars 配置文件 - 混合模式 v2
# 使用 mmdet3d 模型 + 修改后的 mmlite 数据集
# mmlite 数据集输出点云，mmdet3d 的 Det3DDataPreprocessor 进行体素化
...
# ===================== 数据配置 =====================
dataset_type = 'MMLiteKittiDataset'  # mmlite 的 KittiDataset（使用别名避免与 mmdet3d 冲突）
data_root = 'data/kitti/'
class_names = ['Car', 'Pedestrian', 'Cyclist']

# 使用 mmdet3d 的 transforms（不进行 voxelization，让 data_preprocessor 处理）
train_pipeline = [
    dict(
    ...
    # 使用 mmdet3d 的 Pack3DDetInputs 打包数据
    dict(
...
# ===================== 评估配置 =====================
# 使用 mmlite 的简化 KittiMetric（不需要相机信息）
val_evaluator = dict(
    type='mmlite.evaluation.KittiMetric',
