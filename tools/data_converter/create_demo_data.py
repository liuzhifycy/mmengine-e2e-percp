...
Usage:
    python tools/data_converter/create_demo_data.py --num-samples 50
"""
...
def generate_random_boxes(
    ...
    Returns:
        boxes: (N, 7) array [x, y, z, w, l, h, rot]
        labels: (N,) array of class labels
        names: list of class names
    """
    classes = ["Car", "Pedestrian", "Cyclist"]
    class_dims = {
        "Car": ([1.6, 3.9, 1.56], 0.3),  # (mean [w,l,h], std)
        "Pedestrian": ([0.6, 0.8, 1.73], 0.15),
        "Cyclist": ([0.6, 1.76, 1.73], 0.2),
    }
    ...
    for _ in range(num_boxes):
        ...
        # Random dimensions with some variation
        w = max(0.3, mean_dims[0] + np.random.randn() * std)
        l = max(0.5, mean_dims[1] + np.random.randn() * std)
        h = max(0.5, mean_dims[2] + np.random.randn() * std)

        # Random rotation
        rot = np.random.uniform(-np.pi, np.pi)

        boxes.append([x, y, z, w, l, h, rot])
...
def create_info_pkl(
    ...
    for idx, (sample_id, boxes, labels, names) in enumerate(
        ...
        info = {
            ...
            "annos": {
                "name": names,
                "gt_bboxes_3d": boxes.tolist(),
                "gt_labels_3d": labels.tolist(),
                "difficulty": [1] * len(names),  # moderate
            },
        }
        infos.append(info)
...
def main():
    ...
    print("=" * 60)
    print("Creating Demo KITTI Data")
    print("=" * 60)
    ...
    # Create info pkl files
    print("Creating info pkl files...")
